...
## แผนขยายตลาด: จากร้านทำเล็บ → ระบบจองคิวสำหรับร้านที่มีคิวเป็นรอบๆ ทุกประเภท (2026-07-10)

**เป้าหมาย**: เดิมระบบวางตัวขายเฉพาะร้านทำเล็บ (กลุ่มเป้าหมายแคบ) ต้องการขยายไปขายร้านนวด, ร้านตัดผม/ทำผม, สปา, ร้านล้างรถ และร้านคิวรอบๆ อื่นๆ ที่ใช้กลไกเดียวกัน (จองเวลา + มัดจำ + จัดการคิว)

### สรุปการวิเคราะห์
Core engine (slot/template แบบ round-based, hold 10 นาที, deposit, wallet) **เป็นกลางอยู่แล้ว ไม่ผูกกับร้านทำเล็บจริงๆ** — สิ่งที่ผูกกับร้านทำเล็บคือ "คำเรียก" ในหน้า UI (ภาษาไทย เช่น "ช่างทำเล็บ", "ทำเล็บ", emoji 💅 default) และชื่อไฟล์/table/route ที่ตั้งด้วยคำว่า `nail` (~9,000 LOC, ~15+ ไฟล์: `backend/routes/nail.py`, `models.py` 10 ตาราง `nail_*`, หน้า `BookingPage.tsx` / `NailAdminPage.tsx` / `NailSuperAdminPage.tsx`)

### แนวทางที่เลือก: **ปรับที่ "ชั้นคำศัพท์/ธีม" ไม่ต้อง rename โครงสร้างข้อมูล**
เพราะการ rename ตาราง/route ทั้งหมด (`nail_*` → ชื่อกลาง) มีความเสี่ยงสูง (breaking change ต่อ production บน Render + ข้อมูลจริงบน Neon) และไม่ได้เพิ่มมูลค่าให้ลูกค้าโดยตรง เทียบกับการทำให้ระบบ "ดูและพูดเป็นภาษาของแต่ละร้าน" ได้เร็วกว่ามาก จึงเลือกวิธีนี้ก่อน:

1. **เพิ่มฟิลด์ `business_type`** ที่ `Shop`/`NailShopSettings` (ค่าที่เป็นไปได้: `nail`, `hair`, `massage`, `spa`, `carwash`, `other`) — ให้ตอน superadmin สร้างร้านใหม่เลือกประเภทธุรกิจ
2. **สร้าง terminology/label config** (map `business_type` → ชุดคำ: ชื่อพนักงาน/ช่าง, ชื่อบริการ, emoji เริ่มต้น, tagline เริ่มต้น) แล้วให้ทุกหน้า (`BookingPage.tsx`, `NailAdminPage.tsx`) อ่านคำจาก config นี้แทนการ hardcode "ช่างทำเล็บ"/💅 ตรงๆ — ร้านยังปรับแก้เองได้เหมือนเดิมในหน้า Admin (ไม่ล็อกตาม type)
3. **ชื่อฐานข้อมูล/ตาราง/route ที่มีคำว่า `nail` (`nail_*` tables, `/api/nail/*` routes, ไฟล์ `NailAdminPage.tsx` ฯลฯ ยังคงเดิม** — ถือเป็น "ชื่อภายใน" (legacy technical name) ไม่กระทบลูกค้า ไม่คุ้มความเสี่ยงที่จะ rename ตอนนี้ — พิจารณา rename เฉพาะถ้าในอนาคตกลายเป็นปัญหาจริงต่อการดูแลโค้ด (ทำเป็น task แยกทีหลัง ไม่เร่งด่วน)
4. **Rebrand หน้าขาย/การตลาด**: เปลี่ยนข้อความจาก "ระบบจองคิวร้านทำเล็บ" → "ระบบจองคิวออนไลน์สำหรับร้านที่ต้องนัดเวลาเป็นรอบๆ" (นวด, ร้านตัดผม/ทำผม, สปา, ร้านล้างรถ, ทำเล็บ) — ราคาเดิม ไม่เปลี่ยน

### สถานะ
- ยังไม่ implement — เป็นแผนที่ตกลงไว้ ให้ทำเป็นงานถัดไปเมื่อพร้อม (เพิ่ม `business_type`, terminology config, ปรับหน้าสร้างร้านที่ superadmin, ปรับข้อความหน้าขาย)
- ถ้าจะ rename table/route ทั้งระบบในอนาคต ให้แยกเป็น task ต่างหาก และทำเป็น migration แบบ backward-compatible (เช่น เพิ่ม view/alias ชั่วคราว) เพราะมีข้อมูลจริงบน production (Neon) อยู่แล้ว

---

## Rebrand: CSC — Chain System Care (2026-07-11)

หน้า `/superadmin` เปลี่ยนหัวข้อจาก "Super Admin Nail Booking System" → **"CSC Super Admin" / "Chain System Care"** สะท้อนว่าระบบรองรับร้านคิวรอบๆ ทุกประเภทแล้ว ไม่ใช่แค่ร้านทำเล็บ (เปลี่ยนเฉพาะข้อความหน้าจอ ไม่ได้แตะชื่อ table/route ภายในซึ่งยังเป็น `nail_*` เหมือนเดิมตามแผนด้านบน)

### ปรับ UX/UI หน้า `/superadmin` ให้ใช้งานง่ายขึ้นเมื่อร้านเยอะ
- แยกหน้าเป็นแท็บ: **ร้านทั้งหมด / สถานะร้าน / คำขอต่ออายุ / การเงิน / การใช้งาน** — เดิมเป็นหน้ายาวเดียว เลื่อนไกลมากเมื่อมีร้าน 10+ ร้าน
- ลิสต์ "ร้านทั้งหมด" เพิ่มช่องค้นหา (ชื่อ/slug) และแบ่งหน้า (pagination, 8 ร้าน/หน้า) — ปรากฏเมื่อมีร้านมากกว่า 5 ร้าน กันไม่ให้ลิสต์ยาวเกินไป
- แท็บมี badge ตัวเลขแสดงจำนวนร้านทั้งหมด และจำนวนคำขอต่ออายุที่รอดำเนินการ (สีเหลือง) ให้เห็นชัดโดยไม่ต้องเข้าไปดูในแท็บ
- ไฟล์: `artifacts/store/src/pages/NailSuperAdminPage.tsx`

### สรุปผลตรวจสอบความปลอดภัย `/superadmin` (2026-07-11)
ระบบมีการป้องกันหลายชั้นอยู่แล้ว: PIN (`NAIL_SUPER_ADMIN_KEY`, เทียบแบบ constant-time) → OTP 6 หลักส่งผ่าน Telegram (2FA, หมดอายุ 5 นาที) → ออก JWT session (อายุ 12 ชม., เซ็นด้วย secret เดียวกัน) → ทุก endpoint ตรวจ JWT ทุกครั้ง มี rate limit 10 ครั้ง/5 นาทีที่ขั้นตอน login (กัน brute-force ทั้ง PIN และ OTP) และการลบร้านต้องยืนยันซ้ำด้วย OTP ใหม่ + พิมพ์ slug ให้ตรงเป๊ะ ห้ามลบร้านหลัก (id=1)
ไม่ได้แก้โค้ดส่วนความปลอดภัย เพราะโครงสร้างเดิมแข็งแรงอยู่แล้ว — ดูรายละเอียดที่แจ้งผู้ใช้ในแชท
