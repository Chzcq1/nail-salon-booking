...
- Any endpoint accepting a client-supplied FK id that references a `nail_*` row (e.g. `slot_id`, `service_id` in walk-in booking creation) must re-query that row filtered by the caller's `shop_id` and reject if not found — don't just persist the raw id, or a crafted request can link a booking to another shop's slot/service.
